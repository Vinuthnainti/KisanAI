...
import React, { useState, useCallback } from 'react';import { useLanguage } from '@/contexts/LanguageContext';import { EligibilityFilters, isValidAge, MIN_AGE, MAX_AGE } from '@/hooks/useSchemeFilter';

interface EligibilityFormProps {filters: EligibilityFilters;onFilterChange: (filterKey: keyof EligibilityFilters, value: any) => void;onApply: () => void;onReset: () => void;isDarkMode?: boolean;}

// State options for Indiaconst INDIA_STATES = ['Andhra Pradesh','Arunachal Pradesh','Assam','Bihar','Chhattisgarh','Goa','Gujarat','Haryana','Himachal Pradesh','Jharkhand','Karnataka','Kerala','Madhya Pradesh','Maharashtra','Manipur','Meghalaya','Mizoram','Nagaland','Odisha','Punjab','Rajasthan','Sikkim','Tamil Nadu','Telangana','Tripura','Uttar Pradesh','Uttarakhand','West Bengal',];

export const EligibilityForm: React.FC<EligibilityFormProps> = ({filters,onFilterChange,onApply,onReset,isDarkMode = false,}) => {const { translations } = useLanguage();const [validationError, setValidationError] = useState<string>('');

const [ageInput, setAgeInput] = useState<string>(filters.age !== undefined ? String(filters.age) : '');const [ageError, setAgeError] = useState<string>('');

const handleAgeChange = useCallback((rawValue: string) => {setAgeInput(rawValue);
...
const handleApply = useCallback(() => {// Block submission until the Age field (if filled in) is validif (ageError) {setValidationError(ageError);return;}
...
const handleReset = useCallback(() => {setValidationError('');setAgeInput('');setAgeError('');onReset();}, [onReset]);

const bgClass = isDarkMode ? 'bg-gray-800' : 'bg-white';const inputBgClass = isDarkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900';const labelClass = isDarkMode ? 'text-gray-200' : 'text-gray-700';const textClass = isDarkMode ? 'text-gray-300' : 'text-gray-600';const sectionBgClass = isDarkMode ? 'bg-gray-750' : 'bg-gray-50';

return (<div className={p-6 ${bgClass} space-y-6}>{/* Validation Error */}{validationError && (<div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg"><p className="text-red-700 dark:text-red-200 text-sm">{validationError}</p></div>)}
